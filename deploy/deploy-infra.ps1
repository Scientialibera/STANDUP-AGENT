...
$responsesContainer = $config.storage.responses_container
...
foreach ($container in @($responsesContainer, $configContainer)) {
...
if ($deployOpenAI) {
    ...
    $depExists = az cognitiveservices account deployment list --name $openAiAcct --resource-group $rg --query "[?name=='$($config.openai.chat_deployment)'] | length(@)" -o tsv
    if ($depExists -eq "0") {
        az cognitiveservices account deployment create `
          --name $openAiAcct --resource-group $rg `
          --deployment-name $config.openai.chat_deployment `
          --model-format OpenAI --model-name $config.openai.chat_model --model-version $config.openai.chat_model_version `
...
$entraAppName = Select-Value $config.entra.app_display_name "Standup Agent"
...
if ([string]::IsNullOrWhiteSpace($configuredAppId)) {
    ...
    if ([string]::IsNullOrWhiteSpace($existingApp)) {
        $appId = az ad app create --display-name $entraAppName --sign-in-audience AzureADMyOrg --query appId -o tsv
    } else {
        $appId = $existingApp
...
if ([string]::IsNullOrWhiteSpace($botExists)) {
    az bot create `
      --resource-group $rg --name $botName --kind registration `
      --app-type SingleTenant --appid $appId `
...
# 7. Web App Managed Identity + RBAC
...
$settings = @(
    ...
    "BLOB_RESPONSES_CONTAINER=$responsesContainer",
    "BLOB_CONFIG_CONTAINER=$configContainer",
